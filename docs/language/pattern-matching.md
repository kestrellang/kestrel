...
Range patterns currently work with integers only. Character ranges are parsed but require the Char type to be implemented in the type system. Both bounds must be literals of the same type.
