...
fn lang_ty(name: &str) -> AstType {
    AstType::Named {
        segments: vec![
            PathSegment { name: "lang".into(), type_args: vec![], span: Span::synthetic(0) },
            PathSegment { name: name.into(), type_args: vec![], span: Span::synthetic(0) },
        ],
...
fn param_ty(name: &str) -> AstType {
    AstType::Named {
        segments: vec![PathSegment { name: name.into(), type_args: vec![], span: Span::synthetic(0) }],
        span: Span::synthetic(0),
    }
}
...
fn ptr_of(inner: AstType) -> AstType {
    AstType::Named {
        segments: vec![
            PathSegment { name: "lang".into(), type_args: vec![], span: Span::synthetic(0) },
            PathSegment { name: "ptr".into(), type_args: vec![inner], span: Span::synthetic(0) },
        ],
        span: Span::synthetic(0),
    }
}
...
fn seed_fn(world: &mut World, lang: Entity, name: &str, params: &[(&str, AstType)], ret: AstType) {
    ...
    world.set(e, Callable {
        params: params.iter().map(|(n, ty)| AstParam {
            label: None, name: n.to_string(), ty: Some(ty.clone()), default_entity: None, pattern: None, is_mut: false, is_consuming: false,
        }).collect(),
        receiver: None,
    });
...
fn seed_integer_ops(world: &mut World, lang: Entity) {
    ...
    for ty_name in int_types {
        ...
        for op in ["add", "sub", "mul", "and", "or", "xor", "shl"] {
            seed_fn(world, lang, &format!("{ty_name}_{op}"),
                &[("a", ty.clone()), ("b", ty.clone())], ty.clone());
        }

        // Binary comparison ops returning i1
        for op in ["eq", "ne"] {
            seed_fn(world, lang, &format!("{ty_name}_{op}"),
                &[("a", ty.clone()), ("b", ty.clone())], i1.clone());
        }

        // Signed binary ops: arithmetic returns same type, comparison returns i1
        for op in ["div", "rem", "shr"] {
            seed_fn(world, lang, &format!("{ty_name}_signed_{op}"),
                &[("a", ty.clone()), ("b", ty.clone())], ty.clone());
        }
        ...
        for op in ["div", "rem", "shr"] {
            seed_fn(world, lang, &format!("{ty_name}_unsigned_{op}"),
                &[("a", ty.clone()), ("b", ty.clone())], ty.clone());
        }
        for op in ["lt", "le", "gt", "ge"] {
            seed_fn(world, lang, &format!("{ty_name}_unsigned_{op}"),
...
fn seed_float_ops(world: &mut World, lang: Entity) {
    ...
    for ty_name in float_types {
        ...
        // Unary
        seed_fn(world, lang, &format!("{ty_name}_neg"),
            &[("a", ty.clone())], ty.clone());
        ...
        seed_fn(world, lang, &format!("{ty_name}_fma"),
            &[("a", ty.clone()), ("b", ty.clone()), ("c", ty.clone())], ty.clone());

        // copysign(a, b) → same type
        seed_fn(world, lang, &format!("{ty_name}_copysign"),
            &[("a", ty.clone()), ("b", ty.clone())], ty.clone());
    }
}
...
fn seed_cast_ops(world: &mut World, lang: Entity) {
    ...
    for from in &all_types {
        for to in &all_types {
            ...
            seed_fn(world, lang, &format!("cast_{from}_{to}"),
                &[("value", lang_ty(intrinsic_ty(from)))], lang_ty(intrinsic_ty(to)));
        }
    }
}
...
fn seed_pointer_ops(world: &mut World, lang: Entity) {
    ...
    seed_generic_fn(world, lang, "ptr_from_address",
        &[("addr", i64_ty.clone())], ptr_t.clone());
    seed_generic_fn(world, lang, "ptr_to",
        &[("value", t.clone())], ptr_t.clone());
    seed_generic_fn(world, lang, "ptr_read",
        &[("ptr", ptr_t.clone())], t.clone());
    seed_generic_fn(world, lang, "ptr_write",
        &[("ptr", ptr_t.clone()), ("value", t.clone())], unit_ty());
    seed_generic_fn(world, lang, "ptr_offset",
        &[("ptr", ptr_t.clone()), ("offset", i64_ty.clone())], ptr_t.clone());
    ...
    seed_generic_fn(world, lang, "cast_ptr",
        &[("ptr", ptr_any)], ptr_t.clone());
    ...
    seed_generic_fn(world, lang, "ptr_is_null",
...
fn seed_misc_ops(world: &mut World, lang: Entity) {
    ...
    let t = param_ty("T");
    let ptr_t = ptr_of(t.clone());
    seed_generic_fn(world, lang, "atomic_add",
        &[("ptr", ptr_t.clone()), ("value", t.clone())], t.clone());
    seed_generic_fn(world, lang, "atomic_sub",
        &[("ptr", ptr_t), ("value", t.clone())], t);
}
