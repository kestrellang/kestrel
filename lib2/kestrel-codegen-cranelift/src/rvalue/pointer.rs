...
pub fn compile_pointer_op1(
    ...
    match op {
        ...
        Op::PtrRead(ty) => {
            ...
            if is_aggregate(&concrete_ty, &mut ctx.layouts) && !common::type_has_unresolved_params(&concrete_ty) {
...
pub fn compile_stack_alloc(
    ...
    let count = match arg {
        Value::Immediate(Immediate { kind: ImmediateKind::IntLiteral { value, .. } }) => {
            if *value < 0 {
                return Err(CodegenError::Unsupported(format!(
