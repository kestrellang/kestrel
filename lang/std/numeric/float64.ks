    ...
    FFISafe
{
    ...
    public var bitPattern: UInt64 { get {
        UInt64(raw: lang.f64_to_bits(self.raw))
    }}
    ...
    /// ```
    public init(bitPattern bits: UInt64) {
        self.raw = lang.f64_from_bits(bits.raw)
    }
    ...
    public init(parsing string: String)? {
        ...
        if len == 4 {
            ...
            if isMinus and isI and isN and isF {
                self.raw = lang.f64_neg(lang.f64_infinity());
                return
            }
        }
    ...
    public func format(mutating into writer: StringBuilder, options: FormatOptions = FormatOptions.default()) {
        ...
        if self.isNaN {
            ...
            let bits = UInt64(raw: lang.f64_to_bits(value.raw));
            let rawExp = Int64(from: bits.shiftRight(by: 52).bitwiseAnd(UInt64(from: 2047)));
            let rawMant = bits.bitwiseAnd(UInt64(from: 4503599627370495));
            var m = UInt64.zero;
            var e: Int64 = 0;
            if rawExp == 0 {
                m = rawMant;
                ...
                m = rawMant.bitwiseOr(UInt64(from: 4503599627370496));
                e = rawExp - 1075
            }
