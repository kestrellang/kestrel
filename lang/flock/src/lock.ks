...
public func parseLockFile(source source: String) -> Result[LockFile, FlockError] {
    match parseToml(source) {
        .Err(e) => .Err(FlockError.ManifestParse("invalid lock file: " + e.description())),
        .Ok(root) => {
...
public func generateLockFile(entries entries: Array[LockEntry]) -> String {
    ...
    while i < entries.count {
        ...
        buf = buf + "\n[[package]]\n";
        buf = buf + "name = \"" + entry.name + "\"\n";
        buf = buf + "version = \"" + entry.version.toString() + "\"\n";
        buf = buf + "source = \"" + entry.source + "\"\n";
