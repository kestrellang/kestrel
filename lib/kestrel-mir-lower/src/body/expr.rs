...
//!
//! Every arm returns a `ValueId`. Key OSSA properties:
...
impl OssaBodyCtx<'_, '_> {
    ...
    pub fn lower_expr_for_return(&mut self, expr_id: HirExprId) -> ValueId {
        ...
        if let Some(hir_local) = local {
            ...
            let owns_storage =
                self.body.value(addr).ownership == kestrel_mir::value::Ownership::Owned;
            if owns_storage
                && self.ownership_for(ty) == kestrel_mir::value::Ownership::Owned
    ...
    fn lower_assign(
        ...
        match target_expr {
            ...
            HirExpr::Field {
                ...
                let struct_entity = match self.ctx.module.ty_arena.get(base_ty) {
                    kestrel_mir::MirTy::Named { entity, .. } => Some(*entity),
                    _ => None,
                };
                let field_idx = struct_entity
