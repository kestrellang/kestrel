...
//!
//! Uses save/restore via `SavedState` struct to swap BodyCtx state for
//! the closure body. This is necessary because the closure body shares
//! the parent's HirBody arena — we can't create a fresh BodyCtx without
//! conflicting borrows on the parent.
...
use kestrel_mir_2::item::function::{FunctionDef, FunctionKind, ParamDef};
use kestrel_mir_2::item::struct_def::{FieldDef, StructDef};
use kestrel_mir_2::{
    BlockId, FieldIdx, LocalId, MirTy, Operand, ParamConvention, Place, Rvalue, TyId, UseMode,
};
...
impl BodyCtx<'_, '_> {
    pub fn lower_closure_expr(
        ...
        let env_local = closure_body.add_local(LocalDef::new("env", env_ty));
        func_def.params.push(ParamDef::new("env", env_local, env_ty, ParamConvention::Consuming));
        ...
        // Capture locals (loaded from env struct in entry block)
