...
use kestrel_mir_3::mono::{MonoFunction, MonoModule};
use kestrel_mir_3::ImmediateKind;
            ...
            match &init.kind {
                ImmediateKind::IntLiteral { bits, value } => {
                    let bytes = match bits {
                        kestrel_mir_3::IntBits::I8 => vec![*value as u8],
                        kestrel_mir_3::IntBits::I16 => (*value as i16).to_le_bytes().to_vec(),
                        kestrel_mir_3::IntBits::I32 => (*value as i32).to_le_bytes().to_vec(),
                        kestrel_mir_3::IntBits::I64 => (*value as i64).to_le_bytes().to_vec(),
                    };
