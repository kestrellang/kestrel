...
impl TypeOracle for SemanticModel {
    fn resolve_member(
        ...
        if let TyKind::TypeParameter(type_param) = receiver_ty.kind() {
            ...
            let bounds = self.query(TypeParameterBounds {
                        ...
                        for ext in &proto_extensions {
                                ...
                                ext.metadata().get_behavior::<ExtensionTargetBehavior>()
                            {
                                ...
                                if !where_clause.constraints().is_empty()
                                    && !check_where_clause_satisfied_with_self(
                                        self,
                                        where_clause,
                                        proto_subs,
                                        Some(receiver_ty),
        ...
        let member_symbol = match member_symbol {
            ...
            None => {
                ...
                let mut extension_matches: Vec<(usize, Arc<dyn Symbol<KestrelLanguage>>)> =
                    Vec::new();
                for ext in extensions.iter() {
                    if !is_extension_applicable_to_type(
                        ext,
                        &substitutions,
                        self,
                        Some(receiver_ty),
                ...
                match extension_member {
                    ...
                    None => {
                        ...
                        if let Some(resolution) =
                            resolve_member_via_protocol_conformance(self, receiver_ty, member, None)
                        {
                            return Ok(resolution);
                        }
    ...
    fn resolve_member_with_arity(
        ...
        resolve_member_with_arity_and_labels_impl(
            self,
            receiver_ty,
            member,
            is_static,
...
impl TypeOracle for ContextualOracle<'_> {
    ...
    fn conforms_to(&self, ty: &Ty, protocol_id: SymbolId) -> bool {
        ...
        if let TyKind::TypeParameter(type_param) = ty.kind() {
            let mut bounds = self.model.query(TypeParameterBounds {
                param_id: type_param.metadata().id(),
            });
    ...
    fn normalize_with_constraints(&self, ty: &Ty) -> Ty {
        ...
        self.model
            .query(NormalizeWithConstraints::new(ty, self.context_symbol_id))
    }
...
fn resolve_member_with_context(
    ...
    if let TyKind::TypeParameter(type_param) = receiver_ty.kind() {
        // Get bounds by walking up the parent chain to find where clauses
        let mut bounds = model.query(TypeParameterBounds {
            param_id: type_param.metadata().id(),
        });
...
fn resolve_member_full_impl(
    ...
    if scored.is_empty() {
        ...
        if let Some(resolution) =
            resolve_member_via_protocol_conformance(model, receiver_ty, member, Some(labels.len()))
        {
            return Ok(resolution);
        }
        return Err(MemberError::NotFound {
...
fn resolve_init_impl(
    ...
    if argument_types.iter().any(|t| t.is_none()) && scored.len() >= 2 && scored[0].0 == scored[1].0
    {
        return Err(MemberError::UnknownType);
    }
...
fn resolve_subscript_impl(
    ...
    for subscript in &subscript_symbols {
        ...
        let where_constraints =
            get_where_clause_from_symbol(subscript.as_ref()).unwrap_or_default();
    ...
    for &candidate_id in candidates {
        ...
        let (self_ty, parent_subs, parent_type_param_ids) = if is_enum_case {
            if let Some(parent) = symbol.metadata().parent() {
                ...
                (
                    Ty::unit(kestrel_span::Span::synthetic(0)),
                    Substitutions::new(),
                    vec![],
                )
            }
...
fn resolve_all_members_impl(
    ...
    if results.is_empty() {
        if let Some(resolution) =
            resolve_member_via_protocol_conformance(model, receiver_ty, member, None)
        {
            results.push(resolution);
        }
    }
...
fn get_type_container_with_subs(
    ...
    match ty.kind() {
        ...
        TyKind::TypeAlias {
            ...
            if let Some(resolved) = model.query(ResolvedAliasedType {
    ...
    match container.kind() {
        ...
        TyKind::AssociatedType { symbol, container } => {
            if let Some(base_container) = container {
                ...
                model
                    .query(ResolveAssociatedTypeQuery::new(
                        base_container,
                        &base_assoc_name,
                    ))
        ...
        TyKind::AssociatedType { symbol, container } => {
            if let Some(container_ty) = container {
                ...
                let resolved_container =
                    deeply_resolve_associated_types(oracle, container_ty, visited);
                ...
                if let Some(resolved_assoc) = oracle
...
pub(crate) fn collect_protocol_conformances_for_type(model: &SemanticModel, ty: &Ty) -> Vec<Ty> {
    ...
    let applicable_extensions =
        filter_applicable_extensions_for_conformance(Some(model), &extensions, &actual_subs_opt);
    ...
    while let Some(conf) = queue.pop_front() {
        ...
        for (proto, proto_subs) in collect_protocols_with_inherited(proto, proto_subs, Some(&ty)) {
            let inherited_ty =
                Ty::generic_protocol(proto.clone(), proto_subs.clone(), conf.span().clone());
                            ...
                            for ext_conf_ty in ext_conformances.conformances() {
                                ...
                                for candidate_conf in &conformances {
                                    if let TyKind::Protocol {
                                        ...
                                        && candidate_proto.metadata().id()
                                            == ext_proto.metadata().id()
                                    {
...
pub(crate) fn bound_protocols_include(
    model: &SemanticModel,
    bounds: &[Ty],
...
fn resolve_all_associated_types_impl(
    ...
    let result = match ty.kind() {
        ...
        _ => {
            ty.map_children(&mut |child| resolve_all_associated_types_impl(oracle, child, visited))
        },
    };
...
fn resolve_callable_member(
    ...
    let method_type_param_ids =
        if let Some(f) = member_symbol.as_any().downcast_ref::<FunctionSymbol>() {
            f.type_parameters()
                .iter()
...
pub(crate) fn normalize_type_with_context(
    model: &SemanticModel,
    ty: &Ty,
    context_id: SymbolId,
) -> Ty {
    ...
    let qualified_ty = if !owned_equalities.is_empty()
    ...
    let equalities: Vec<(&Ty, &Ty)> = owned_equalities.iter().map(|(l, r)| (l, r)).collect();
    ...
    while changed && iterations < MAX_ITERATIONS {
        ...
        match current.kind().clone() {
            ...
            TyKind::Struct {
                ...
                let (new_subs, inner_changed) = normalize_substitutions(&substitutions, equalities);
                if inner_changed {
...
fn type_contains_param(ty: &Ty, param_id: SymbolId) -> bool {
    ...
    fn inner(ty: &Ty, param_id: SymbolId, visited: &mut HashSet<TyId>) -> bool {
        ...
        match ty.kind() {
            ...
            TyKind::Tuple(elements) => elements.iter().any(|elem| inner(elem, param_id, visited)),
            TyKind::Pointer(element) => inner(element, param_id, visited),
            TyKind::Function {
                params,
                return_type,
            } => {
                params.iter().any(|p| inner(p, param_id, visited))
