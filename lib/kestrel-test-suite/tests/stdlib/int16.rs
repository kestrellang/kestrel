...
// TODO: Fails due to unary minus requiring Negatable protocol resolution on Int16
#[test]
fn int16_overflow_behavior() {
    Test::new(
        ...
        func main() -> lang.i64 {
            let maxVal = std.num.Int16.maxValue;
            let minVal = std.num.Int16.minValue;
            let one = std.num.Int16(intLiteral: 1);
            let negOne = std.num.Int16(intLiteral: -1);
            ...
            let hundred = std.num.Int16(intLiteral: 100);
            let fifty = std.num.Int16(intLiteral: 50);
            let addNormal = hundred.addChecked(fifty);
            if addNormal.isNone() { return 2 }
            if addNormal.unwrap() != std.num.Int16(intLiteral: 150) { return 3 }
            ...
            if subNormal.unwrap() != std.num.Int16(intLiteral: 50) { return 6 }

            // multiplyChecked — overflow near boundaries
            let big = std.num.Int16(intLiteral: 20000);
            let two = std.num.Int16(intLiteral: 2);
            let mulOverflow = big.multiplyChecked(two);
            if mulOverflow.isSome() { return 7 }

            // multiplyChecked — normal case
            let ten = std.num.Int16(intLiteral: 10);
            let five = std.num.Int16(intLiteral: 5);
            let mulNormal = ten.multiplyChecked(five);
            if mulNormal.isNone() { return 8 }
            if mulNormal.unwrap() != std.num.Int16(intLiteral: 50) { return 9 }
            ...
            if negHundred.unwrap() != std.num.Int16(intLiteral: -100) { return 12 }
            ...
            let negFifty = std.num.Int16(intLiteral: -50);
            ...
            let addSatBig = maxVal.addSaturating(std.num.Int16(intLiteral: 1000));
            ...
            let negBig = std.num.Int16(intLiteral: -20000);
            ...
            if negSatHundred != std.num.Int16(intLiteral: -100) { return 24 }
...
fn int16_bitwidth_and_conversion() {
    Test::new(
        ...
        func main() -> lang.i64 {
            ...
            let val = std.num.Int16(intLiteral: 258);
            ...
            let zero = std.num.Int16(intLiteral: 0);
            ...
            let val256 = std.num.Int16(intLiteral: 256);
            if val256.leadingZeros != 7 { return 6 }

            // rotateLeft — 16-bit rotation
            if one.rotateLeft(by: 1) != std.num.Int16(intLiteral: 2) { return 7 }
            let fortyTwo = std.num.Int16(intLiteral: 42);
            ...
            let testVal = std.num.Int16(intLiteral: 12345);
            ...
            if fromI64 != std.num.Int16(intLiteral: 1000) { return 12 }

            // init(from:) — from Int8
            let i8val = std.num.Int8(intLiteral: -50);
            let fromI8 = std.num.Int16(from: i8val);
            if fromI8 != std.num.Int16(intLiteral: -50) { return 13 }

            // parse — valid Int16 value
            let parsed = std.num.Int16.parse(string: "1000");
            if parsed.isNone() { return 14 }
            if parsed.unwrap() != std.num.Int16(intLiteral: 1000) { return 15 }

            // parse — negative value
            let parsedNeg = std.num.Int16.parse(string: "-32768");
            if parsedNeg.isNone() { return 16 }
            if parsedNeg.unwrap() != std.num.Int16.minValue { return 17 }

            // parse — maxValue
            let parsedMax = std.num.Int16.parse(string: "32767");
            if parsedMax.isNone() { return 18 }
            if parsedMax.unwrap() != std.num.Int16.maxValue { return 19 }

            // parse — out-of-range (too large)
            let parsedBig = std.num.Int16.parse(string: "32768");
            if parsedBig.isSome() { return 20 }

            // parse — out-of-range (too small)
            let parsedSmall = std.num.Int16.parse(string: "-32769");
            if parsedSmall.isSome() { return 21 }

            // parse — invalid string
            let parsedBad = std.num.Int16.parse(string: "xyz");
            if parsedBad.isSome() { return 22 }

            // parse — empty string
            let parsedEmpty = std.num.Int16.parse(string: "");
