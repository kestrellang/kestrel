...
module Test
        ...
        func main() -> lang.i64 {
            ...
            let lit0: std.numeric.UInt64 = 0;
            if minVal.isEqual(to: lit0) == false { return 1 }

            // maxValue is 18446744073709551615
            let maxVal = std.numeric.UInt64.maxValue;
            let lit18446744073709551615: std.numeric.UInt64 = 18446744073709551615;
            if maxVal.isEqual(to: lit18446744073709551615) == false { return 2 }

            // bitWidth is 64
            if std.numeric.UInt64.bitWidth != 64 { return 3 }

            // zero constant
            let z = std.numeric.UInt64.zero;
            let zeroLit: std.numeric.UInt64 = 0;
            if z.isEqual(to: zeroLit) == false { return 4 }

            // one constant
            let o = std.numeric.UInt64.one;
            let oneLit: std.numeric.UInt64 = 1;
