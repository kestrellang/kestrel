...
module Test
        ...
        func main() -> lang.i64 {
            ...
            let lit72057594037927936: std.numeric.UInt64 = 72057594037927936;
            if one.byteSwapped.isEqual(to: lit72057594037927936) == false { return 1 }

            // byteSwapped — double swap is identity
            let val: std.numeric.UInt64 = 123456789;
            if val.byteSwapped.byteSwapped.isEqual(to: val) == false { return 2 }

            // leadingZeros — relative to 64-bit width
            if one.leadingZeros != 63 { return 3 }

            let zero: std.numeric.UInt64 = 0;
            if zero.leadingZeros != 64 { return 4 }

            // Value with high bit set: 2^63 = 9223372036854775808
            let highBit: std.numeric.UInt64 = 9223372036854775808;
            if highBit.leadingZeros != 0 { return 5 }

            // rotateLeft — 64-bit rotation
            // rotateLeft(1, by: 1) = 2
            let rotTwo: std.numeric.UInt64 = 2;
            ...
            let testVal: std.numeric.UInt64 = 123456789;
            if testVal.rotateLeft(by: 17).rotateRight(by: 17).isEqual(to: testVal) == false { return 10 }

            // init(from:) — from Int64
            let fromI64Val: std.numeric.Int64 = 1000000;
            let fromI64 = std.numeric.UInt64(from: fromI64Val);
            let lit1000000: std.numeric.UInt64 = 1000000;
            if fromI64.isEqual(to: lit1000000) == false { return 11 }

            // init(from:) — from UInt8
            let fromU8Val: std.numeric.UInt8 = 255;
            let fromU8 = std.numeric.UInt64(from: fromU8Val);
            let lit255: std.numeric.UInt64 = 255;
