...
module Test
        ...
        func main() -> lang.i64 {
            // ---- reduce() ----
            let sum = [1, 2, 3, 4].iter().reduce(combining: { (a, b) in a + b });
            if sum.isNone() { return 1 }
            if sum.unwrap() != 10 { return 2 }

            // reduce on single element
            let single = [42].iter().reduce(combining: { (a, b) in a + b });
            ...
            let none = empty.iter().reduce(combining: { (a, b) in a + b });
            if none.isSome() { return 5 }

            // reduce for max
            let maxVal = [3, 1, 4, 1, 5].iter().reduce(combining: { (a, b) in if a > b { a } else { b } });
