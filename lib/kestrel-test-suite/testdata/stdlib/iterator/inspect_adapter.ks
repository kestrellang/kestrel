...
module Test
        ...
        func main() -> lang.i64 {
            ...
            let result: std.collections.Array[std.numeric.Int64] = [1, 2, 3].iter().inspect(inspecting: { (x) in }).collect();
            ...
            let inspected = [1, 2, 3, 4, 5].iter().inspect(inspecting: { (x) in });
            ...
            let emptyResult = empty.iter().inspect(inspecting: { (x) in }).collect();
