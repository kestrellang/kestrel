...
module Test
        ...
        func main() -> lang.i64 {
            ...
            let result: std.collections.Array[std.numeric.Int64] = [1, 2, 3].iter().intersperseWith({ () in 0 }).collect();
            ...
            let single: std.collections.Array[std.numeric.Int64] = [42].iter().intersperseWith({ () in 0 }).collect();
            ...
            let result2: std.collections.Array[std.numeric.Int64] = [10, 20, 30].iter().intersperseWith({ () in -1 }).collect();
