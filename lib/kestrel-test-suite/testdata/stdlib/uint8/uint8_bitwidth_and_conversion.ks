...
module Test
        ...
        func main() -> lang.i64 {
            ...
            if val.byteSwapped.isEqual(to: val) == false { return 1 }
            let maxVal = std.numeric.UInt8.maxValue;
            if maxVal.byteSwapped.isEqual(to: maxVal) == false { return 2 }
            ...
            if v128.rotateLeft(by: 1).isEqual(to: one) == false { return 7 }
            ...
            if one.rotateRight(by: 1).isEqual(to: v128) == false { return 9 }

            // rotateLeft and rotateRight are inverses
            let testVal: std.numeric.UInt8 = 42;
            if testVal.rotateLeft(by: 3).rotateRight(by: 3).isEqual(to: testVal) == false { return 10 }
            ...
            if fromI64.isEqual(to: lit200) == false { return 11 }
