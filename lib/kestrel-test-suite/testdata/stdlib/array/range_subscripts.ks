...
module Test
        ...
        func main() -> lang.i64 {
            ...
            let slice = arr(std.core.Range[std.numeric.Int64](1, 4));
            ...
            let oob = arr(checked: std.core.Range[std.numeric.Int64](0, 10));
            if oob.isSome() { return 13 }

            // subscript(checked:) — Range arg, negative start
            let negStart = arr(checked: std.core.Range[std.numeric.Int64](-1, 3));
            if negStart.isSome() { return 14 }

            // subscript(unchecked:) — Range arg
            let unchecked = arr(unchecked: std.core.Range[std.numeric.Int64](2, 4));
            ...
            let clamped = arr(clamped: std.core.Range[std.numeric.Int64](1, 3));
            ...
            let clampedWide = arr(clamped: std.core.Range[std.numeric.Int64](-5, 100));
            ...
            let clampedPast = arr(clamped: std.core.Range[std.numeric.Int64](10, 20));
            if clampedPast.count != 0 { return 24 }

            // subscript(clamped:) - negative range clamped to start
            let clampedNeg = arr(clamped: std.core.Range[std.numeric.Int64](-5, 1));
