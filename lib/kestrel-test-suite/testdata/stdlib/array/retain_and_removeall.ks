...
module Test
        ...
        func makeArray5() -> std.collections.Array[std.numeric.Int64] {
        ...
        func main() -> lang.i64 {
            ...
            var arr2 = makeArray5();
            arr2.removeAll(where: { (x) in x % 2 == 0 });
            ...
            arr4.retain(where: { (x) in false });
            if arr4.count != 0 { return 9 }

            // removeAll on empty array
            var arr5 = std.collections.Array[std.numeric.Int64]();
            arr5.removeAll(where: { (x) in true });
