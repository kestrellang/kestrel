...
module Test
        ...
        func main() -> lang.i64 {
            ...
            if it.next().unwrap().equals("beta") == false { return 3 }
            if it.next().isSome() { return 4 }
            // Underlying byte range includes the terminator after "beta"
            if sub.toString().equals("alpha\nbeta\n") == false { return 5 }

            // ---- ClosedRange matches half-open with +1 end ----
            let subClosed = s.lines(std.core.ClosedRange[std.num.Int64](0, 1));
            if subClosed.toString().equals("alpha\nbeta\n") == false { return 6 }

            // ---- whole buffer round-trip ----
            let whole = s.lines(std.core.Range[std.num.Int64](0, 3));
            if whole.toString().equals("alpha\nbeta\ngamma") == false { return 7 }

            // ---- CRLF preservation ----
            let crlf: std.text.String = "a\r\nb\r\nc";
            let crlfSub = crlf.lines(std.core.Range[std.num.Int64](0, 1));
            if crlfSub.toString().equals("a\r\n") == false { return 8 }
            ...
            if trailTail.toString().equals("b") == false { return 13 }
            ...
            let last = s.lines(std.core.Range[std.num.Int64](2, 3));
            if last.toString().equals("gamma") == false { return 16 }

            // ---- past-end: checked returns None ----
            let oob = s.lines(checked: std.core.Range[std.num.Int64](0, 999));
            if oob.isSome() { return 17 }

            // ---- past-end: clamped saturates to count ----
            let clamp = s.lines(clamped: std.core.Range[std.num.Int64](-5, 999));
            if clamp.toString().equals("alpha\nbeta\ngamma") == false { return 18 }

            // ---- negative start, checked ----
            let negCheck = s.lines(checked: std.core.Range[std.num.Int64](-1, 2));
            ...
            if s.lines.substring(std.core.ClosedRange[std.num.Int64](0, 1)).equals("alpha\nbeta\n") == false { return 24 }
