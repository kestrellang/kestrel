    ...
    fn stdlib_passes_verifier() {
        ...
        for func in &mir.functions {
            if let Some(body) = &func.body {
                ...
                let errors =
                    kestrel_mir_3::verify::verify_ossa(body, &mir, &func.name, func.entity);
                if !errors.is_empty() {
                ...
                for err in &errors {
                    if err.message.contains("live at block exit") {
                        ...
                        if err.block.index() == 0 {
                            ...
                            term,
                            kestrel_mir_3::terminator::TerminatorKind::Return(_)
                        ) {
    ...
    fn stdlib_passes_pipeline() {
        ...
        let shim_count = mir
            ...
            .filter(|f| {
                matches!(
                    f.kind,
                    kestrel_mir_3::item::function::FunctionKind::DropShim { .. }
                )
            })
            .count();
        let thunk_count = mir
            ...
            .filter(|f| {
                matches!(
                    f.kind,
                    kestrel_mir_3::item::function::FunctionKind::Thunk { .. }
                )
            })
            .count();
    ...
    fn stdlib_monomorphize() {
        ...
        match mono_result {
            Ok(mut mono_module) => {
                ...
                kestrel_mir_3::mono::expand::expand_destroy_copy(
                    &mut mono_module,
                    &generic_functions,
                );
