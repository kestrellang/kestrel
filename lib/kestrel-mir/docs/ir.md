...
Ownership is a property of the value definition rather than something
reconstructed by dataflow analysis: an SSA value is created once with a
fixed ownership kind and is never reassigned, so its ownership state is
known structurally at every use.
...
OSSA blocks have typed, ownership-annotated parameters. These carry
ownership across control-flow merge points structurally, so no runtime
drop flags are needed.
...
The call-target model carries `Thin` and `Thick` variants that point at
`ValueId`s:
...
`SwitchCase` has these variants: `Wildcard`, `Variant(VariantIdx)`,
...
The IR design follows from a handful of decisions, each of which keeps an
invariant structural in the IR rather than recovered by a separate analysis:

| Design | Why |
|--------|-----|
| `ValueId` SSA values | Ownership is structural, not inferred |
| `CopyValue` / `MoveValue` instructions | Copies are explicit in the IR |
| `CallArg { convention }` + explicit borrows | Borrows have scope in the IR |
| `DestroyValue` emitted by the lowerer | No analysis pass needed |
| Block arguments at merge points | No runtime drop flags |
| Loops thread values via header block params | Liveness across iterations is explicit |
| `BasicBlock.params: Vec<BlockParam>` | Ownership at joins is structural |
