...
- [ ] Method-chain receivers (`a.b.c.`, `f().`, `(expr).`) in completion. Phase 5's HIR walk only fires when the smallest covering `HirExpr::Field` is a bare receiver — chained / parenthesised dots fall through to scope completion. Needs a CST-based locator that finds the FieldAccess whose `Missing` member contains the cursor, then asks inference for the base's resolved type.
