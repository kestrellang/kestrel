...
values need ownership tracking.

MIR-2 tracked all types through init_state and liveness dataflow, then
filtered at the point of use. OSSA skips trivial values entirely.

## Partial Moves

Unlike MIR-2 which prohibited partial moves (verifier rejected
`Use(Place(s.f), Move)` for droppable aggregates), OSSA supports
partial extraction through destructuring:
...
extract all fields it needs and destroy the rest. This is equivalent to
MIR-2's "whole-aggregate move" but more flexible.

For the initial implementation, the lowerer can continue to use
whole-aggregate moves (`move_value` the entire struct) and defer partial
extraction to a future optimization. The IR supports it; the lowerer
doesn't have to use it immediately.

## How Each MIR-2 Bug Is Fixed

### Bug 1: Pointer.read() use-after-free

**MIR-2**: `Pointer.read()` returns a copy. Drop_elab treats the copy as
independently owned and drops it, destroying shared resources.

**OSSA**: A plain `Load` is valid only for trivial `@none` values. For
non-trivial memory, the lowerer must choose one explicit operation:
`copy_addr` to create an independent owned copy, `take` to move the value
out and leave memory uninitialized, or `begin_borrow_addr` for a
temporary guaranteed borrow. The verifier rejects a non-trivial `Load`,
so the IR cannot accidentally create two owners for the same memory.

### Bug 2: Transitive drop detection failure (8KB/req leak)

**MIR-2**: `drop_elab` maintains a "droppable set" and misses types
whose fields are droppable but whose TypeInfo doesn't enumerate them.

**OSSA**: No droppable set. Every @owned value must be explicitly
consumed. If the lowerer forgets to emit `destroy_value`, the verifier
catches it ("unconsumed @owned value at block exit"). Leaks are
impossible if the verifier passes.

### Bug 3: Call-result temp leaks / double-frees

**MIR-2**: Call results excluded from droppable set → leak. Included →
double-free (because clone_elab doesn't handle temp-to-temp copies).

**OSSA**: Call results are new @owned ValueIds. They must be consumed
exactly once. Move to another instruction → consumed. Not moved →
must destroy. The verifier catches both leaks and double-frees.

### Bug 4: Drop flag complexity

**MIR-2**: `DropIf` + `SetDropFlag` + `drop_flag_expand` span the
entire function, with fragile ordering between flag updates and drops.

**OSSA**: Block arguments replace flags. At every merge point, each
predecessor explicitly passes owned values or destroys them. The CFG
encodes ownership flow — no runtime flags needed.
