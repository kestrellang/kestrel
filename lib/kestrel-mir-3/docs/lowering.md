...
How the HIR→OSSA lowerer emits ownership-correct IR. This is the hardest
part of the migration — the lowerer must emit copy_value, destroy_value,
begin_borrow/end_borrow, and block arguments correctly from the start.

## Architecture

The current MIR-2 lowerer (`kestrel-mir-lower/src/body_lower.rs`, 5,499
lines) has excellent separation of concerns:

1. **Single statement emission choke point**: `emit_stmt()` (line 272)
2. **Single value-mode decision point**: `emit_value_transfer()` (line 5342)
3. **Modular control flow handlers**: `lower_if`, `lower_loop`, `lower_match`
   are independent, self-contained functions
4. **Existing scope tracking**: `local_scopes`, `ScopeLive` markers

The OSSA lowerer can reuse this architecture. The key changes are:
- Values are `ValueId`s instead of `Place(LocalId)`
- `emit_value_transfer` emits `CopyValue`/`MoveValue` instead of `UseMode`
- Control flow handlers thread complete owned live-in sets through block
  arguments instead of writing to shared locals
...
### emit_value_transfer replacement

MIR-2's `emit_value_transfer` (line 5342 in body_lower.rs) is the
single canonicalized site where Move/Copy becomes an Rvalue. In OSSA:
...
MIR-2 pattern (uses shared result local):
```rust
// MIR-2
let result_local = fresh_temp(result_ty);
// then arm: assign to result_local, jump merge
// else arm: assign to result_local, jump merge
// merge: read result_local
```

OSSA pattern (uses block arguments):
```rust
...
Key difference: the result flows through block arguments, not a shared
mutable local. In real lowering, the merge block also receives any
`@owned` values still live after the `if` expression. Each arm either
passes those values to the merge block or consumes/destroys them before
jumping. The block parameter list is the complete owned live-in set, not
just the expression result.

### Loops

MIR-2 uses ScopeId::Loop + ScopeLive to reset init-state at loop
headers. OSSA uses block arguments to thread live values through the
loop:
...
## What the Current Lowerer Already Has

The current lowerer (`body_lower.rs`) already has clean architecture
that maps well to OSSA:

| Current feature | OSSA equivalent |
|-----------------|-----------------|
| `emit_stmt()` choke point | `emit()` for OSSA instructions |
| `emit_value_transfer()` | Copy vs Move decision point |
| `arg_for_value()` | Convention-based borrow decision |
| `apply_callee_param_modes()` | Emit begin_borrow/end_borrow |
| `local_scopes` HashMap | Scope frame stack |
| `ScopeLive` markers | Block arg threading at loop headers |
| `fresh_temp()` | `new_value()` |
| `switch_to_block()` | Same |
| `set_terminator()` | Same, with block args |

The architecture is compatible. The main new work is:
1. Scope frame stack (~100 lines)
2. Block argument threading in control flow handlers (~500 lines)
3. Borrow insertion around calls (~200 lines)
4. Destroy emission at scope exits (~100 lines)
