...
use kestrel_mir_2::{ImmediateKind, MonoFunction, MonoModule};
...
impl<'m> CodegenCtx<'m> {
    ...
    pub fn is_main_function(&self, func: &MonoFunction) -> bool {
        ...
        // Match "main" or "<module>.main" but not "<Type>.main" (methods).
        // Module-level main has exactly one dot (e.g. "app.main").
        name == "main"
            || (name.ends_with(".main")
    ...
    fn define_static(&mut self, s: &kestrel_mir_2::MonoStatic) -> Result<(), CodegenError> {
        ...
        // Check for immediate initializer
        if let Some(init) = &s.initializer {
            match &init.kind {
                ImmediateKind::IntLiteral { bits, value } => {
                    let bytes = match bits {
                        kestrel_mir_2::IntBits::I8 => vec![*value as u8],
                        kestrel_mir_2::IntBits::I16 => (*value as i16).to_le_bytes().to_vec(),
                        kestrel_mir_2::IntBits::I32 => (*value as i32).to_le_bytes().to_vec(),
                        kestrel_mir_2::IntBits::I64 => (*value as i64).to_le_bytes().to_vec(),
                    };
    ...
    fn define_file_constant(
        &mut self,
        s: &kestrel_mir_2::MonoStatic,
        fcd: &kestrel_mir_2::item::static_def::FileConstantData,
        _size: usize,
    ) -> Result<(), CodegenError> {
        // Read file bytes
        ...
        // Store count as little-endian at offset ptr_size
        ...
        // Add relocation for the pointer
        let raw_gv = self
            .cl_module
