...
use kestrel_mir_2::{FloatBits, ImmediateKind, MonoFuncId};
...
pub fn compile_immediate(
    ...
    match kind {
        ...
        ImmediateKind::FloatLiteral { bits, value } => match bits {
            FloatBits::F16 => {
                // No direct f16const; use f32 and demote
...
/// Build a Str fat pointer {data_ptr, len} on the stack.
fn compile_string_literal(
    ...
    // Allocate Str struct {ptr, len} on stack
...
/// Return just a raw pointer to string data (no fat pointer).
